import type { ArmedModes } from '../models/Area.js'
import type { OmniLinkPlatform } from '../platform.js'
import type { ApplicationDataResponse } from './messages/ApplicationDataResponse.js'
import type { SystemStatusResponse } from './messages/SystemStatusResponse.js'

import { EventEmitter } from 'node:events'

import { AccessControl, AccessControlLockStatus } from '../models/AccessControl.js'
import { Area, AreaStatus, ExtendedArmedModes } from '../models/Area.js'
import { AudioSource } from '../models/AudioSource.js'
import { AudioZone, AudioZoneStatus } from '../models/AudioZone.js'
import { AuxiliarySensor, AuxiliarySensorStatus } from '../models/AuxiliarySensor.js'
import { Button } from '../models/Button.js'
import { Code } from '../models/Code.js'
import { OmniObjectModel, OmniObjectStatusTypes, SystemTroubles } from '../models/OmniObjectModel.js'
import { OmniTemperature } from '../models/OmniTemperature.js'
import { SystemFormats, TemperatureFormats } from '../models/SystemFormats.js'
import { SystemInformation } from '../models/SystemInformation.js'
import { SystemStatus } from '../models/SystemStatus.js'
import { FanModes, HoldStates, Thermostat, ThermostatModes, ThermostatStatus } from '../models/Thermostat.js'
import { Unit, UnitStatus } from '../models/Unit.js'
import { Zone, ZoneStatus } from '../models/Zone.js'
...
import { ControllerCommandRequest } from './messages/ControllerCommandRequest.js'
import { EnableNotificationsRequest } from './messages/EnableNotificationsRequest.js'
import { AudioZoneCommandStates, AuthorityLevels, Commands, EmergencyTypes, MessageTypes, ObjectStatusTypes, ObjectTypes }
  from './messages/enums.js'
...
import { ExtendedObjectStatusRequest } from './messages/ExtendedObjectStatusRequest.js'
...
import { KeypadEmergencyRequest } from './messages/KeypadEmergencyRequest.js'
import { ObjectPropertiesRequest } from './messages/ObjectPropertiesRequest.js'
import { ObjectTypeCapacitiesRequest } from './messages/ObjectTypeCapacitiesRequest.js'
import { ObjectTypeCapacitiesResponse } from './messages/ObjectTypeCapacitiesResponse.js'
import { SecurityCodeValidationRequest } from './messages/SecurityCodeValidationRequest.js'
import { SecurityCodeValidationResponse } from './messages/SecurityCodeValidationResponse.js'
import { SetTimeCommandRequest } from './messages/SetTimeCommandRequest.js'
import { SystemFormatsRequest } from './messages/SystemFormatsRequest.js'
import { SystemFormatsResponse } from './messages/SystemFormatsResponse.js'
import { SystemInformationRequest } from './messages/SystemInformationRequest.js'
import { SystemInformationResponse } from './messages/SystemInformationResponse.js'
import { SystemStatusRequest } from './messages/SystemStatusRequest.js'
import { SystemTroublesRequest } from './messages/SystemTroublesRequest.js'
...
import { OmniSession } from './OmniSession.js'
...
export class OmniService extends EventEmitter {
  ...
  private syncTimeIntervalId?: NodeJS.Timeout | undefined
  ...
  async setTime(): Promise<void> {
    ...
    try {
      ...
      now = new Date(Math.round(Date.now() / 60000) * 60000)
  ...
  async setThermostatFanMode(thermostatId: number, fanMode: FanModes): Promise<void> {
    this.platform.log.debug(this.constructor.name, 'setThermostatFanMode', thermostatId, fanMode)

    const thermostat = this.omni.thermostats[thermostatId]
    try {
      const message = new ControllerCommandRequest({
        command: Commands.SetThermostatFanMode,
        parameter1: fanMode,
        parameter2: thermostatId,
      })

      if (this.platform.settings.showOmniEvents) {
        this.platform.log.info(`${thermostat}: Set Fan Mode ${FanModes[fanMode]}`)
      }
      ...
      if (error instanceof Error) {
        this.platform.log.warn(`${thermostat}: Set Fan Mode failed [${error.message}]`)
      }
    }
  }
  ...
  private async getObjectStatus(
    ...
    startId: number,
    endId: number,
  ): Promise<ApplicationDataResponse | undefined> {
  ...
  private async getCodeId(areaId: number): Promise<number | undefined> {
    ...
    try {
      ...
      if (!(response instanceof SecurityCodeValidationResponse)) {
        throw new TypeError('SecurityCodeValidationResponse not received')
      }
