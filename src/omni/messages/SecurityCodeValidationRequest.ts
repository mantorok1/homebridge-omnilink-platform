...
interface SecurityCodeValidationRequestArgs {
  areaId: number
  code: string
}
...
export class SecurityCodeValidationRequest extends ApplicationDataRequest {
  ...
  get data(): Buffer {
    return Buffer.from([
      this.type,
      this.areaId,
      parseInt(this.code.substr(0, 1)),
      parseInt(this.code.substr(1, 1)),
      parseInt(this.code.substr(2, 1)),
      parseInt(this.code.substr(3, 1)),
    ])
  }
}
