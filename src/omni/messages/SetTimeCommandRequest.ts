...
interface SetTimeCommandRequestArgs {
  year: number
  month: number
  day: number
  dayOfWeek: number
...
export class SetTimeCommandRequest extends ApplicationDataRequest {
  ...
  get data(): Buffer {
    return Buffer.from([this.type, this.year, this.month, this.day, this.dayOfWeek, this.hour, this.minute, this.daylightSavings])
  }
}
