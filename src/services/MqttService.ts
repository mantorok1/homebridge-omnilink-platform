...
import { Alarms, ExtendedArmedModes } from '../models/Area.js'
import { OmniObjectStatusTypes } from '../models/OmniObjectBase.js'
import { SystemTroubles } from '../models/OmniObjectModel.js'
import { TemperatureFormats } from '../models/SystemFormats.js'
import { FanModes, HoldStates, ThermostatModes, ThermostatStates } from '../models/Thermostat.js'
...
export class MqttService {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      for (const [areaId, area] of this.platform.omniService.omni.areas.entries()) {
        ...
        this.platform.omniService.on(this.platform.omniService.getEventKey(OmniObjectStatusTypes.Area, areaId), this.publishArea.bind(this, areaId))
      ...
      for (const [zoneId, zone] of this.platform.omniService.omni.zones.entries()) {
        ...
        this.platform.omniService.on(this.platform.omniService.getEventKey(OmniObjectStatusTypes.Zone, zoneId), this.publishZone.bind(this, zoneId))
      ...
      for (const [unitId, unit] of this.platform.omniService.omni.units.entries()) {
        ...
        this.platform.omniService.on(this.platform.omniService.getEventKey(OmniObjectStatusTypes.Unit, unitId), this.publishUnit.bind(this, unitId))
      ...
      for (const [thermostatId, thermostat] of this.platform.omniService.omni.thermostats.entries()) {
        ...
        this.subTopics.set(`${this.prefix}thermostat/${thermostatId}/humidifysetpoint/set`, this.setThermostatHumidifySetPoint.bind(this))
        this.subTopics.set(`${this.prefix}thermostat/${thermostatId}/dehumidifysetpoint/set`, this.setThermostatDehumidifySetPoint.bind(this))
        ...
        this.platform.omniService.on(this.platform.omniService.getEventKey(OmniObjectStatusTypes.Thermostat, thermostatId), this.publishThermostat.bind(this, thermostatId))
      ...
      for (const [accessControlId, accessControl] of this.platform.omniService.omni.accessControls.entries()) {
        ...
        this.platform.omniService.on(this.platform.omniService.getEventKey(OmniObjectStatusTypes.AccessControlLock, accessControlId), this.publishLock.bind(this, accessControlId))
      ...
      for (const [sensorId, sensor] of this.platform.omniService.omni.sensors.entries()) {
        this.platform.omniService.on(this.platform.omniService.getEventKey(OmniObjectStatusTypes.AuxiliarySensor, sensorId), this.publishAuxiliarySensor.bind(this, sensorId))
  ...
  publishThermostat(thermostatId: number, status: ThermostatStatus) {
    ...
    this.publish(`thermostat/${thermostatId}/name/get`, this.platform.omniService.omni.thermostats[thermostatId].name)

    this.publish(`thermostat/${thermostatId}/mode/get`, ThermostatModes[status.mode].toLowerCase())

    this.publish(`thermostat/${thermostatId}/fan/get`, FanModes[status.fan].toLowerCase())

    this.publish(`thermostat/${thermostatId}/hold/get`, HoldStates[status.hold].toLowerCase())

    this.publish(`thermostat/${thermostatId}/temperature/get`, String(status.temperature.toFormat()))

    this.publish(`thermostat/${thermostatId}/coolsetpoint/get`, String(status.coolSetPoint.toFormat()))

    this.publish(`thermostat/${thermostatId}/heatsetpoint/get`, String(status.heatSetPoint.toFormat()))

    this.publish(`thermostat/${thermostatId}/state/get`, ThermostatStates[status.state].toLowerCase())

    this.publish(`thermostat/${thermostatId}/humidity/get`, String(status.humidity.toFormat()))

    this.publish(`thermostat/${thermostatId}/humidifysetpoint/get`, String(status.humidifySetPoint.toFormat()))

    this.publish(`thermostat/${thermostatId}/dehumidifysetpoint/get`, String(status.dehumidifySetPoint.toFormat()))
  }
  ...
  publishLock(accessControlId: number, lockStatus: AccessControlLockStatus) {
    ...
    this.publish(`accesscontrol/${accessControlId}/name/get`, this.platform.omniService.omni.accessControls[accessControlId].name)

    this.publish(`accesscontrol/${accessControlId}/locked/get`, String(lockStatus.locked))
  }
